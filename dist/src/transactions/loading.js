...
exports.loadHistoryTxs = void 0;
const ethers_1 = require("ethers");
const provider = ethers_1.ethers.providers.getDefaultProvider(process.env.REACT_APP_ETHEREUM_NODE_URL);
...
const erc20Interface = new ethers_1.ethers.utils.Interface(erc20InterfaceDefinition);
const erc20OldInterface = new ethers_1.ethers.utils.Interface(erc20OldInterfaceDefinition);
const erc721Interface = new ethers_1.ethers.utils.Interface(erc721InterfaceDefinition);
...
const safeInterface = new ethers_1.ethers.utils.Interface(safeInterfaceDefinition);
...
const loadEthTx = async (txHash) => {
    return await provider.getTransaction(txHash);
};
const decodeTx = (account, tx) => {
    try {
...
const mutlisigTxEntry = async (account, log, safeTxHash, success, subLogs) => {
    ...
    return Object.assign(Object.assign({ type: "Multisig", id: "multisig_" + safeTxHash, timestamp: block.timestamp, logs: subLogs, txHash: log.transactionHash, safeTxHash,
        success }, decodedTx), { nonce: -1 // We don't know yet
     });
};
const moduleTxEntry = async (log, moduleAddress, success, subLogs) => {
    const block = await loadBlock(log.blockHash);
    return {
        type: "Module",
        txHash: log.transactionHash,
...
const transferEntry = async (account, log) => {
    const block = await loadBlock(log.blockHash);
    let type = "";
    let eventInterface;
...
const mapLog = async (account, log, subLogs) => {
    switch (log.topics[0]) {
        case successTopic: {
        ...
        case transferTopic: {
            if (subLogs.length > 0)
                console.error("Sub logs for transfer entry!", log, subLogs);
...
const loadOutgoingTransfer = (account) => {
    const filter = {
        topics: [[transferTopic], [ethers_1.ethers.utils.defaultAbiCoder.encode(["address"], [account])]],
        fromBlock: "earliest"
    };
...
const loadIncomingTransfer = (account) => {
    const filter = {
        topics: [[transferTopic], null, [ethers_1.ethers.utils.defaultAbiCoder.encode(["address"], [account])]],
        fromBlock: "earliest"
    };
    return provider.getLogs(filter).then((e) => {
...
const loadSafeTransactionTransfer = (account) => {
    const filter = {
...
const mergeLogs = async (...loaders) => {
    ...
    if (loaderCount == 1)
        return logResults[0];
    const currentLogIndex = new Array(loaderCount).fill(0);
...
const groupIdFromLog = (log) => `${log.blockNumber}_${log.transactionIndex}`;
const updateGroupedLogs = (groups, parentCandidate, currentChildren) => {
    if (parentCandidate) {
        groups.push({
...
const loadHistoryTxs = async (account, start) => {
    ...
    return (await Promise.all(inter)).filter((e) => e !== undefined);
};
exports.loadHistoryTxs = loadHistoryTxs;
